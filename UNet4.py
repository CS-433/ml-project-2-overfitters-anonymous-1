...
class UNet4(nn.Module):
    def __init__(
        ...
        self.bottleneck = self.down_convolution_block(level=4, kernel_size=conv_kernel_size, stride=conv_stride)
        self.up_sampling_4 = self.up_sampling(level=4) # scale=2 => image size is multiplied by 2
    ...
    def down_convolution_block(self, level, kernel_size, stride):
        ...
        # what changes from down_convolution_block to up_convolution_block is the way number_of_in/out_channels are computed.
        
        kernel_size = int(kernel_size - (kernel_size + 1 )%2)   # to make sure kernel size is odd
        padding = int((kernel_size - 1)*0.5)                    # padding such that output image has same spatial size
    ...
    def up_convolution_block(self, level, kernel_size, stride):
        ...
        kernel_size = int(kernel_size - (kernel_size + 1 )%2)   # to make sure kernel size is odd
        padding = int((kernel_size - 1)*0.5)                    # padding sucht that output image has same spatial size
    ...
    def up_sampling(self, level):
        # transpose convolution, halves the number of out channels. The other half will come from skip connections.
        # Dobbles the spatial size 
